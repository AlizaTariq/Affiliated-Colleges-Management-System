...
function Settings() {
  ...
  const handleEditPersonal = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:5001/UpdateExaminer",
        formData,
        { headers: headers }
      );
  ...
  const handleExaminerQualification = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:5001/ExaminerQualification",
        formData,
        { headers: headers }
      );
  ...
  const handleExaminerExper = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:5001/ExaminerExperience",
        formData,
        { headers: headers }
      );
      if (response.data["status"] === "fail") {
  ...
  const fetchExperience = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:5001/NewExperience", {
        headers: headers,
      });
  ...
  const fetchQualificatoins = async () => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:5001/NewQualifications",
        { headers: headers }
      );
  ...
  const fetchCourses = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:5001/AllCourses", {
        headers: headers,
      });
  ...
  const handleSubmitInterest = async (event) => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:5001/UpdateExaminerCourse",
        { data: textInput },
        { headers: headers }
      );
      if (response.data["status"] === "fail") {
          ...
          <form onSubmit={handleEditPersonal}>
            ...
            <div className="PersonalEditForm">
              ...
              <input
                type="text"
                className="PersonalEditInput"
                placeholder="Enter Phone Number (+92 0000000000)"
                name="phone"
                value={usr_phone}
            ...
            <div className="PersonalEditForm">
              ...
              <select
                className="form-label designLable"
                name="gender"
                value={usr_gender}
            ...
            <div style={{ textAlign: "right" }}>
              <button type="submit" className="PersonalEditSubmitt">
                Update data
              </button>
            </div>
            <div>{errorP && <div className="update_msj">{errorP}</div>}</div>
          </form>
        </div>
        <h2>Interests</h2>
        <div className="PersonalForm">
          <div className="option_">
            <form onSubmit={handleSubmitInterest}>
              ...
              <div>{errorI && <div className="update_msj">{errorI}</div>}</div>
            </form>
          </div>
        </div>
        <h2>Qualification</h2>
        ...
        <div id="AddNewQualification" className="Setting-Modal">
          ...
          >
            ...
            <div>
              <form
                style={{ width: "90%" }}
                onSubmit={handleExaminerQualification}
              >
                <div className="mainSettdiv">
                  <span></span>
                  <input
                    type="text"
                    className="inputQuabox"
                ...
                <div className="mainSettdiv">
                  <span></span>
                  <label className="label_" htmlFor="starting_date">
                    Starting Date:
                  </label>
                  <input
                    className="form-control inputQuabox"
                    type="date"
                    name="starting_date"
                    runat="server"
                    onChange={(e) => setStartingDateQ(e.target.value)}
                    style={{ height: "30px", width: "fit-content" }}
                  />
                </div>
                <div className="mainSettdiv">
                  <span></span>
                  <label className="label_" htmlFor="ending_date">
                    Ending Date:
                  </label>
                  <input
                    className="form-control inputQuabox"
                    ...
                    name="ending_date"
                    runat="server"
                    onChange={(e) => setEndingDateQ(e.target.value)}
                    style={{ height: "30px", width: "fit-content" }}
                  />
                </div>
                <div className="mainSettdiv">
                  <label className="label_" htmlFor="Certificate">
                    Transcript:{" "}
                  </label>
                  <input
                    type="file"
              ...
              <form style={{ width: "90%" }} onSubmit={handleExaminerExper}>
                ...
                <div className="mainSettdiv">
                  ...
                  <input
                    ...
                    onChange={(e) => set_reference_email(e.target.value)}
                    required
                  />
                </div>
                <div className="mainSettdiv">
                ...
                <div className="mainSettdiv">
                  <label className="label_" htmlFor="ExperianceLetter">
                    Experiance Letter:{" "}
                  </label>
                  <input
                    type="file"
